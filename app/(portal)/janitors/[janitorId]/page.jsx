...
export default function Maintainer({ params }) {
  ...
  const janitorId = params.janitorId;
  ...
  useEffect(() => {
    async function fetchJanitor() {
      try {
        const res = await clientApi.get(`/janitors/${janitorId}`);
        const { name, email, contactNumber, properties, status } = res.data.janitor;
  ...
  const updateInfo = async () => {
    ...
    if (validateInfo(data, setErrors)) {
      try {
        const res = await clientApi.put(`/janitors/${janitorId}/update/info`, data);
  ...
  const updatePassword = async () => {
    ...
    if (validatePassword(data, setErrors)) {
      try {
        const res = await clientApi.put(`/janitors/${janitorId}/update/password`, data);
        showToast(res.data.message, "success");
        setData((prevData) => ({
          ...prevData,
          password: "",
  ...
  const updateProperties = async () => {
    ...
    try {
      ...
      const res = await clientApi.put(`/janitors/${janitorId}/update/properties`, {
        properties,
      });
      showToast(res.data.message, "success");
    } catch (error) {
  ...
  const updateStatus = async () => {
    try {
      const res = await clientApi.put(`/janitors/${janitorId}/update/status`, {
  ...
  const handleDelete = async () => {
    try {
      const res = await clientApi.delete(`/janitors/${janitorId}`);
      showToast(res.data.message, "success");
      setOpenModal(false);
