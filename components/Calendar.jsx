...
export default function Calendar({ selectedDate = moment(), setSelectedDate, minDate, maxDate, notices = [] }) {
  ...
  return (
    <div className="rounded-lg boxshadow-sm bg-light dark:bg-dark p-4">
            ...
            {week.map((day, i) => (
              <div key={i} className={`w-8 h-8 text-center ${currentMonth.isSame(day, "month") ? "" : "text-gray-400"}`}>
                ...
                >
                  <div
                    className={
                      notices.filter((noti) => moment(noti.date).isSame(day, "day")).length > 0
